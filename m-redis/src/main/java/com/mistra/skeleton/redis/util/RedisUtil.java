...
public class RedisUtil {
    ...
    public Map<String, String> batchGetMapAll(String key) {
        return (Map) this.executePipeline((pair) -> {
            ...
            return (Map) (CollectionUtils.isEmpty(mapKeys) ? new HashMap<String, String>() : this.processMap(key, mapKeys, pipeline));
        });
    }
    ...
    private Map<String, String> processMap(String key, Collection<String> mapKeys, Pipeline pipeline) {
        ...
        int index = 0;
        Map<byte[], Response<byte[]>> newMap = new HashMap<byte[], Response<byte[]>>();
    ...
    public <V> V doWorkInPool(PoolWork<Jedis, V> work) {
